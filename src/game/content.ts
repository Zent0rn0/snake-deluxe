import { info, neutral, reward, special } from '../design/tokens';
import type { MutatorId, PowerId } from './types';

export type Rarity = 'common' | 'rare' | 'epic' | 'legendary';

export const RARITY: Record<Rarity, { label: string; color: string }> = {
  common: { label: 'Обычный', color: neutral.soft },
  rare: { label: 'Редкий', color: info.base },
  epic: { label: 'Эпический', color: special.base },
  legendary: { label: 'Легендарный', color: reward.base },
};
...
export interface SkinDef {
  ...
  rarity: Rarity;
  outline: string;
  /** Optional rim light stroked once around the body. Legendaries only. */
  rim?: string;
...
export const SKINS: SkinDef[] = [
  {
    ...
    rarity: 'common',
    outline: '#166534',
    color: (i) => (Math.floor(i / 2) % 2 === 0 ? '#4ade80' : '#3ecf74'),
    preview: ['#4ade80', '#22c55e'],
  },
  {
    id: 'ocean',
    name: 'Океан',
    ...
    rarity: 'common',
    outline: '#1e3a8a',
    color: (i, len) => `hsl(${205 + (i / Math.max(len, 1)) * 30}, 90%, ${62 - (i / Math.max(len, 1)) * 14}%)`,
    preview: ['#38bdf8', '#2563eb'],
  },
  {
    id: 'sunset',
    name: 'Закат',
    ...
    rarity: 'common',
    outline: '#9a3412',
    color: (i, len) => `hsl(${35 - (i / Math.max(len, 1)) * 45}, 95%, 60%)`,
    preview: ['#fb923c', '#ec4899'],
  },
  {
    id: 'bee',
    name: 'Пчёлка',
    ...
    rarity: 'common',
    outline: '#422006',
    color: alt('#facc15', '#292524', 4),
    preview: ['#facc15', '#292524'],
  },
  {
    id: 'candy',
    name: 'Леденец',
    ...
    rarity: 'rare',
    outline: '#9d174d',
    color: alt('#fbcfe8', '#f43f5e', 3),
    preview: ['#fbcfe8', '#f43f5e'],
  },
  {
    id: 'tiger',
    name: 'Тигр',
    ...
    rarity: 'rare',
    outline: '#431407',
    color: (i) => (i % 7 < 2 && i > 3 ? '#1c1917' : '#f97316'),
    preview: ['#f97316', '#1c1917'],
  },
  {
    id: 'zebra',
    name: 'Зебра',
    ...
    rarity: 'rare',
    outline: '#0a0a0a',
    color: alt('#fafafa', '#262626', 3),
    preview: ['#fafafa', '#262626'],
  },
  {
    id: 'watermelon',
    name: 'Арбуз',
    ...
    rarity: 'rare',
    outline: '#14532d',
    color: (i) => (i < 4 ? '#f43f5e' : i % 6 < 3 ? '#16a34a' : '#86efac'),
    preview: ['#16a34a', '#f43f5e'],
  },
  {
    id: 'neon',
    name: 'Неон',
    ...
    rarity: 'epic',
    outline: '#1f9aad',
    color: (i, _l, t) => (Math.floor(i / 3 + t * 6) % 4 === 0 ? '#5fd3e0' : '#0e1726'),
    preview: ['#0e1726', '#1f9aad'],
  },
  {
    id: 'lava',
    name: 'Лава',
  ...
  },
  {
    ...
    rarity: 'epic',
    outline: '#5457c9',
    color: (i, _l, t) => `hsla(${230 + Math.sin(i * 0.4 + t * 2) * 20}, 100%, 88%, 0.9)`,
    preview: ['#e0e7ff', '#818cf8'],
  },
  {
    id: 'rainbow',
    name: 'Радуга',
    ...
    rarity: 'legendary',
    outline: '#312e81',
    color: (i, _l, t) => `hsl(${(i * 14 - t * 120) % 360}, 74%, 60%)`,
    preview: ['#d9556b', '#7f63c4'],
  },
  {
    id: 'galaxy',
    name: 'Галактика',
  ...
  },
  {
    ...
    outline: '#713f12',
    rim: '#e3b04b',
    color: (i, _l, t) => `hsl(45, 82%, ${50 + Math.max(0, Math.sin(i * 0.35 - t * 6)) * 20}%)`,
    preview: ['#e8c65c', '#b07f1c'],
  },
];
...
export interface ThemeDef {
  ...
  frame: string;
  frameLight: string;
  /** Tint applied to the app chrome (`--app-tint`) while this theme is equipped. */
  chromeTint: string;
  decor: string[];
  decorAlpha: number;
  grid?: string;
