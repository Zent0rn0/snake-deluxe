...
export type ControlScheme = 'swipe' | 'joystick' | 'dpad' | 'tap';
...
export interface Settings {
  ...
  reducedFx: boolean;
  turnSound: boolean;
  /** How touch input is read. Ignored when playing on a keyboard. */
  control: ControlScheme;
  /** Which side the movement control sits on. */
  handedness: 'right' | 'left';
  /** 0..1, mapped to a swipe threshold of 18px down to 8px. */
  swipeSensitivity: number;
  haptics: 'off' | 'light' | 'full';
}

/** The v1 shape, read once during migration. */
...
/**
 * v1 → v2: the single `dpad` mode and boolean `vibration` became a control
 * scheme and a three-way haptics setting. The storage key is deliberately
 * untouched — it names the slot, `Profile.v` names the schema, and bumping the
 * key would wipe every existing player's coins, stars and purchases.
 */
export function migrate(parsed: Record<string, unknown>, base: Profile): Profile {
  const merged: Profile = {
    ...base,
    ...(parsed as Partial<Profile>),
    owned: { ...base.owned, ...(parsed.owned as Profile['owned']) },
    equipped: { ...base.equipped, ...(parsed.equipped as Profile['equipped']) },
    settings: { ...base.settings, ...(parsed.settings as Partial<Settings>) },
    stats: { ...base.stats, ...(parsed.stats as Profile['stats']) },
    classicSetup: { ...base.classicSetup, ...(parsed.classicSetup as Profile['classicSetup']) },
    arenaSetup: { ...base.arenaSetup, ...(parsed.arenaSetup as Profile['arenaSetup']) },
    duelSetup: { ...base.duelSetup, ...(parsed.duelSetup as Profile['duelSetup']) },
  };

  if (!(typeof parsed.v === 'number' && parsed.v >= 2)) {
    const old = (parsed.settings ?? {}) as SettingsV1;
    // 'on' meant an always-visible d-pad; 'auto' and 'off' both meant swipes.
    if (old.dpad) merged.settings.control = old.dpad === 'on' ? 'dpad' : 'swipe';
    if (typeof old.vibration === 'boolean') merged.settings.haptics = old.vibration ? 'light' : 'off';
    merged.v = 2;
  }
  // Drop the retired keys so they cannot shadow the new ones later.
  delete (merged.settings as Partial<SettingsV1>).dpad;
  delete (merged.settings as Partial<SettingsV1>).vibration;
  return merged;
}
...
export const ACHIEVEMENTS: AchievementDef[] = [
  { id: 'first', name: 'Первый укус', desc: 'Сыграй первую игру', sprite: 'apple', reward: 20, check: (p) => p.stats.games >= 1 },
  {
    id: 'fruits100',
    name: 'Гурман',
    desc: 'Съешь 100 фруктов',
    sprite: 'grapes',
    reward: 50,
    check: (p) => p.stats.fruits >= 100,
    progress: (p) => [p.stats.fruits, 100],
  },
  {
    id: 'fruits1000',
    name: 'Ненасытный',
    desc: 'Съешь 1000 фруктов',
    sprite: 'watermelon',
    reward: 250,
    check: (p) => p.stats.fruits >= 1000,
    progress: (p) => [p.stats.fruits, 1000],
  },
  {
    id: 'score500',
    name: 'Полтысячи',
    desc: 'Набери 500 очков за игру',
    sprite: 'hundred',
    reward: 60,
    check: (p) => p.stats.bestScore >= 500,
    progress: (p) => [p.stats.bestScore, 500],
  },
  {
    id: 'score1500',
    name: 'Мастер змей',
    desc: 'Набери 1500 очков за игру',
    sprite: 'glowing_star',
    reward: 150,
    check: (p) => p.stats.bestScore >= 1500,
    progress: (p) => [p.stats.bestScore, 1500],
  },
  {
    id: 'combo8',
    name: 'Комбо-мастер',
    desc: 'Съешь 8 фруктов в одном комбо',
    sprite: 'fire',
    reward: 80,
    check: (p) => p.stats.maxCombo >= 8,
    progress: (p) => [p.stats.maxCombo, 8],
  },
  {
    id: 'combo15',
    name: 'В ударе',
    desc: 'Комбо из 15 фруктов',
    sprite: 'collision',
    reward: 160,
    check: (p) => p.stats.maxCombo >= 15,
    progress: (p) => [p.stats.maxCombo, 15],
  },
  {
    id: 'length30',
    name: 'Длинная история',
    desc: 'Вырасти до длины 30',
    sprite: 'snake',
    reward: 80,
    check: (p) => p.stats.maxLength >= 30,
    progress: (p) => [p.stats.maxLength, 30],
  },
  {
    id: 'length60',
    name: 'Анаконда',
    desc: 'Вырасти до длины 60',
    sprite: 'medal',
    reward: 220,
    check: (p) => p.stats.maxLength >= 60,
    progress: (p) => [p.stats.maxLength, 60],
  },
  {
    id: 'golden5',
    name: 'Золотоискатель',
    desc: 'Съешь 5 золотых яблок',
    sprite: 'coin',
    reward: 60,
    check: (p) => p.stats.goldens >= 5,
    progress: (p) => [p.stats.goldens, 5],
  },
  {
    id: 'allpowers',
    name: 'Коллекционер',
    desc: 'Подбери каждый из 6 бонусов',
    sprite: 'gift',
    reward: 120,
  ...
  },
  {
    id: 'portal50',
    name: 'Телепортёр',
    desc: 'Пройди через порталы 50 раз',
    sprite: 'dizzy',
    reward: 80,
    check: (p) => p.stats.portals >= 50,
    progress: (p) => [p.stats.portals, 50],
  },
  ...
  { id: 'arenawin', name: 'Царь арены', desc: 'Победи в арене', sprite: 'crown', reward: 120, check: (p) => p.stats.arenaWins >= 1 },
  {
    id: 'kills10',
    name: 'Хищник',
    desc: 'Съешь 10 соперников в арене',
    sprite: 'skull',
    reward: 100,
    check: (p) => p.stats.kills >= 10,
    progress: (p) => [p.stats.kills, 10],
  },
  { id: 'duel', name: 'Дружеский матч', desc: 'Сыграй дуэль вдвоём', sprite: 'gamepad', reward: 40, check: (p) => p.stats.duels >= 1 },
  {
    id: 'blitz600',
    name: 'Молния',
    desc: 'Набери 600 очков в Блице',
    sprite: 'bolt',
    reward: 100,
    check: (p) => (p.best.blitz ?? 0) >= 600,
    progress: (p) => [p.best.blitz ?? 0, 600],
  },
  {
    id: 'campaign5',
    name: 'Путешественник',
    desc: 'Пройди 5 уровней приключения',
    sprite: 'map',
    reward: 80,
    check: (p) => campaignDone(p) >= 5,
    progress: (p) => [campaignDone(p), 5],
  },
  {
    id: 'campaignAll',
    name: 'Легенда',
    desc: 'Пройди всё приключение',
    sprite: 'trophy',
    reward: 300,
    check: (p) => campaignDone(p) >= LEVELS.length,
    progress: (p) => [campaignDone(p), LEVELS.length],
  },
  {
    id: 'stars45',
    name: 'Перфекционист',
    desc: 'Собери все 45 звёзд',
    sprite: 'star',
    reward: 500,
    check: (p) => campaignStars(p) >= LEVELS.length * 3,
    progress: (p) => [campaignStars(p), LEVELS.length * 3],
  },
  { id: 'daily', name: 'Испытатель', desc: 'Пройди испытание дня', sprite: 'calendar', reward: 60, check: (p) => p.stats.dailyWins >= 1 },
  {
    id: 'blender',
    name: 'Блендер',
    desc: 'Сыграй классику с 3+ модификаторами',
    sprite: 'cyclone',
    reward: 60,
    check: (p) => p.stats.blenderGames >= 1,
  },
  { id: 'shop', name: 'Модник', desc: 'Купи первую вещь в магазине', sprite: 'bags', reward: 30, check: (p) => p.stats.purchases >= 1 },
  {
    id: 'collector',
    name: 'Стилист',
    desc: 'Собери 10 вещей',
    sprite: 'sparkles',
    reward: 150,
    check: (p) => ownedCount(p) >= 10,
    progress: (p) => [ownedCount(p), 10],
  },
  { id: 'wheel', name: 'Везунчик', desc: 'Крутани колесо удачи', sprite: 'slot', reward: 20, check: (p) => p.stats.spins >= 1 },
  {
    ...
    progress: (p) => [levelInfo(p.xp).level, 10],
  },
];
...
export const QUESTS: QuestDef[] = [
  ...
  },
  {
    ...
    reward: 80,
    kind: 'max',
    text: (n) => `Сделай комбо из ${n} фруктов`,
    metric: (r) => r.maxCombo,
  },
  {
    id: 'score',
    ...
    reward: 80,
    kind: 'max',
    text: (n) => `Набери ${n} очков за игру`,
    metric: (r) => (r.mode === 'duel' ? 0 : r.score),
  },
  {
    id: 'powers',
    sprite: 'gift',
    ...
    reward: 70,
    kind: 'sum',
    text: (n) => `Подбери ${n} бонусов`,
    metric: (r) => Object.values(r.powers).reduce((a, b) => a + (b ?? 0), 0),
  },
  {
    id: 'arena',
    sprite: 'swords',
    ...
    kind: 'sum',
    text: () => 'Победи в арене',
    metric: (r) => (r.mode === 'arena' && r.win ? 1 : 0),
  },
  {
    id: 'kills',
    sprite: 'skull',
    ...
    reward: 90,
    kind: 'sum',
    text: (n) => `Съешь ${n} змеек в арене`,
    metric: (r) => (r.mode === 'arena' ? r.kills : 0),
  },
  {
    id: 'blitz',
    sprite: 'bolt',
    ...
    reward: 90,
    kind: 'max',
    text: (n) => `Набери ${n} очков в Блице`,
    metric: (r) => (r.mode === 'blitz' ? r.score : 0),
  },
  { id: 'games', sprite: 'gamepad', targets: [3, 5, 7], reward: 50, kind: 'sum', text: (n) => `Сыграй ${n} игр`, metric: () => 1 },
  {
    ...
    reward: 70,
    kind: 'sum',
    text: (n) => `Съешь ${n} золот${n === 1 ? 'ое яблоко' : 'ых яблока'}`,
    metric: (r) => r.goldens,
  },
  {
    id: 'stars',
    ...
    reward: 90,
    kind: 'sum',
    text: (n) => `Собери ${n} звёзд в приключении`,
    metric: (r) => r.stars,
  },
  {
    id: 'length',
    ...
    reward: 70,
    kind: 'max',
    text: (n) => `Вырасти до длины ${n}`,
    metric: (r) => (r.mode === 'duel' ? 0 : r.length),
  },
  {
    id: 'portals',
    sprite: 'dizzy',
    ...
    reward: 60,
    kind: 'sum',
    text: (n) => `Пройди через портал ${n} раз`,
    metric: (r) => r.portals,
  },
];
...
/**
 * Segment colours follow the token families rather than a pastel rainbow: the
 * small prizes sit in the cool half, the rare ones carry the reward gold so the
 * wheel reads at a glance.
 */
export const WHEEL: WheelPrize[] = [
  { label: '25', sprite: 'coin', coins: 25, color: '#3f6f9e', weight: 22 },
  { label: '50', sprite: 'coin', coins: 50, color: '#6b579b', weight: 20 },
  { label: '100', sprite: 'moneybag', coins: 100, color: '#96557a', weight: 14 },
  { label: '40', sprite: 'coin', coins: 40, color: '#2f8a5b', weight: 20 },
  { label: '250', sprite: 'gem', coins: 250, color: '#b8862f', weight: 6 },
  { label: 'Вещь', sprite: 'gift', coins: 0, color: '#a8463f', weight: 8, item: true },
  { label: '75', sprite: 'coin', coins: 75, color: '#4a7fa8', weight: 14 },
  { label: '500', sprite: 'crown', coins: 500, color: '#c8871f', weight: 2 },
];
...
export function spinWheel(): { index: number; prize: WheelPrize; itemName?: string; achievements: AchievementDef[] } | null {
  ...
  setProfile((p) => {
    ...
    if (prize.item) {
      ...
      const pool = [
        ...lockedSkins.map((s) => ({ kind: 'skins' as const, id: s.id, name: `Скин «${s.name}»` })),
        ...lockedHats.map((h) => ({ kind: 'hats' as const, id: h.id, name: `Шляпа «${h.name}»` })),
      ];
      if (pool.length) {
